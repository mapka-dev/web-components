...
export function SplitPanel({ rightPanel, leftPanel }: SplitPanelProps) {
  ...
  return (
    <>
      <style>
        {`
        #split-panel[data-direction=col] { 
          display: grid;
        ...
        #spilt-panel([data-resizing=true][data-direction=col]){ cursor: col-resize; }
        #split-panel[data-direction=row] { 
          display: grid;
        ...
        #spilt-panel([data-resizing=true][data-direction=row]){ cursor: row-resize; }
        
        #spilt-panel([resizing]){ 
          user-select: none; 
        }
        #split-panel[data-direction=col] #median {
          inline-size: 10px;
          grid-column: 2 / 3; 
          background: grey;
        }
        #split-panel[data-direction=row] #median {
          block-size: 10px;
          grid-row: 2 / 3; 
          background: grey;
        }
        #split-panel[data-direction=col] #median:hover { 
          cursor: col-resize; 
        }
        #split-panel[data-direction=row] #median:hover { 
          cursor: row-resize; 
        }
        #split-panel[data-direction=col] #left-panel { 
          grid-column: 1 / 2; 
          grid-row: 1 / 1;
        }
        #split-panel[data-direction=row] #left-panel { 
          grid-column: 1 / 1; 
          grid-row: 1 / 2;
        }
        #split-panel[data-direction=col] #right-panel { 
          grid-column: 3 / 4; 
          grid-row: 1 / 1; 
        }
        #split-panel[data-direction=row] #right-panel { 
          grid-column: 1 / 1; 
          grid-row: 3 / 4; 
        }
        `}
